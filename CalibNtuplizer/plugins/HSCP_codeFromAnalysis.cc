...
//        if(useClusterCleaning && !clusterCleaning(dedxHits->stripCluster(h), crossTalkInvAlgo))continue;
        ...
        if (useStrip && detid.subdetId()>=3) {
         ...
         if (debugprint) std::cout << " after amps " << std::endl;
         bool_cleaning=clusterCleaning(amps, crossTalkInvAlgo);
        }
        if(useClusterCleaning && !bool_cleaning)continue;
        ...
        if(detid.subdetId()>=3){//for strip only
           ...
           if (debugprint) std::cout << " after amps2 " << std::endl;
        ...
        if (debugprint) std::cout << " after gain corrected " << std::endl;
     ...
     if(size>0){
        if(templateHisto){  //dEdx discriminator
           ...
           if(useTruncated){
              ...
              result /= (size-nTrunc);
              if (debugprint) std::cout << " truncated40 discriminator " << result << std::endl;
           }else{
              //harmonic2 estimator           
              result=0;
              double expo = -2;
