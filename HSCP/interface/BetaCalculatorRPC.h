...
public:
  ...
  // Run algo on RPC RecHits
  void algo(const std::vector<susybsm::RPCHit4D>& HSCPRPCRecHits);

  // Attach RPC beta info to HSCP candidate
...
private:
  ...
  // Token for RPC RecHits
  edm::EDGetTokenT<RPCRecHitCollection> rpcRecHitsToken;

  // ES token for RPC geometry (Run 3 thread-safe framework)
